...
- **小程序虚拟支付（待开发）**：支持道具直购（`short_series_goods`）和代币充值（`short_series_coin`），包含服务端双签名（paySig + signature）及订单回调发货。
...
- ✅ 订单状态机（pending → confirmed → completed）
